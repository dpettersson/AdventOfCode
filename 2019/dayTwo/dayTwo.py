...
# either 1, 2, or 99. The opcode indicates what to do; for example, 99 means that the program is finished and should
...
# but it multiplies instead of adding. The inputs are at positions 3 and 11; these positions contain 70 and 50
...
def process_data(data_list: list):
    ...
    while pos < len(data_list):
        opcode = data_list[pos]

        if opcode == 99:
            return data_list

        var1_pos = data_list[pos + 1]
        var2_pos = data_list[pos + 2]
        dest = data_list[pos + 3]

        if opcode == 1:
            data_list[dest] = data_list[var1_pos] + data_list[var2_pos]
        elif opcode == 2:
            data_list[dest] = data_list[var1_pos] * data_list[var2_pos]
...
assert process_data([1, 0, 0, 0, 99]) == [2, 0, 0, 0, 99]
assert process_data([2, 3, 0, 3, 99]) == [2, 3, 0, 6, 99]
assert process_data([2, 4, 4, 5, 99, 0]) == [2, 4, 4, 5, 99, 9801]
assert process_data([1, 1, 1, 4, 99, 5, 6, 0, 99]) == [30, 1, 1, 4, 2, 5, 6, 0, 99]
...
def day_two_part_one():
    ...
    print("The data at position 0 is " + str(data[0]))
...
def day_two_part_two():
    ...
    for noun in range(0, 99):
        for verb in range(0, 99):
            ...
            if data[0] == target:
                print("The pair Noun: " + str(noun) + " and verb: " + str(verb) + " gives the result 19690720")
                print("The result of  100 * verb + noun is: " + str(100 * noun + verb))
