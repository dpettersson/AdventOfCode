...
def calculate_fuel(mass_of_object):
    return (mass_of_object // 3) - 2
...
def day_one_part_one():
    ...
    for line in input_data:
        fuel_needed += calculate_fuel(int(line))

    print("The total fuel needed is " + str(fuel_needed))
...
def calculate_fuel_need_for_fuel(start_fuel):
    fuel_needed = calculate_fuel(start_fuel)

    if fuel_needed > 0:
        fuel_needed += calculate_fuel_need_for_fuel(fuel_needed)
...
assert calculate_fuel_need_for_fuel(12) == 2
assert calculate_fuel_need_for_fuel(14) == 2
assert calculate_fuel_need_for_fuel(1969) == 966
assert calculate_fuel_need_for_fuel(100756) == 50346
...
def day_one_part_two():
    ...
    for line in input_data:
        current_part_fuel = calculate_fuel(int(line))
        fuel_needed += current_part_fuel + calculate_fuel_need_for_fuel(current_part_fuel)
