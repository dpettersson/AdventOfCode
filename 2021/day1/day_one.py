...
#
# To do this, count the number of times a depth measurement increases from the previous measurement. (There is no m
# easurement before the first measurement.) In the example above, the changes are as follows:
#
...
def day_one_part_one(input_data):
    increases = 0
    current_number = int(input_data[0])
    ...
    print("THere are " + str(increases) + " increases")
    return increases


test_data = [199, 200, 208, 210, 200, 207, 240, 269, 260, 263]

assert day_one_part_one(test_data) == 7

#day_one_part_one(read_input())
...
def day_one_part_two(input_data):
    increases = 0
    index = 1
    last_sum = int(input_data[0])

    while index < len(input_data):
        current_sum = sum(input_data[index: index + 3])
...
#assert day_one_part_two(test_data) == 5
day_one_part_two(read_input())
