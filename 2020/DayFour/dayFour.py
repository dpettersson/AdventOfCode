...
#
# Due to some questionable network security, you realize you might be able to solve both of these problems at the s
# ame time.
#
...
def parse_passport(passport_data):
    ...
    extracted_field_names = [None] * 8

    if passport_data.find('byr:') != -1:
        extracted_field_names[byr_index] = True
    if passport_data.find('iyr:') != -1:
        extracted_field_names[iyr_index] = True
    if passport_data.find('eyr:') != -1:
        extracted_field_names[eyr_index] = True
    if passport_data.find('hgt:') != -1:
        extracted_field_names[hgt_index] = True
    if passport_data.find('hcl:') != -1:
        extracted_field_names[hcl_index] = True
    if passport_data.find('ecl:') != -1:
        extracted_field_names[ecl_index] = True
    if passport_data.find('pid:') != -1:
        extracted_field_names[pid_index] = True
    if passport_data.find('cid:') != -1:
        extracted_field_names[cid_index] = True

    return extracted_field_names


def count_passports(passport_list):
    no_of_correct_passports = 0

    for passport in passport_list:
        if passport.count(True) == 7 and passport[7] is None:  # We got 7 fields. Check if the missing field is cid
            no_of_correct_passports = no_of_correct_passports + 1

    return no_of_correct_passports


def day_four_part_one(data):
    ...
    for line in data:
        if line != "":
            raw_passport_data = raw_passport_data + " " + line
        else:
            passport_list.append(parse_passport(raw_passport_data))
            raw_passport_data = ""

    passport_count = count_passports(passport_list)

    print("Number of passports found: " + str(passport_count))


input_file = open("input.txt", "r")
data = input_file.read().splitlines()

#assert day_four_part_one(test_data) == 2

#day_four_part_one(data)
...
def extract_year(raw_data, offset=4):
    year = 0

    extracted_data = raw_data[offset:]

    if extracted_data.isdigit() and len(extracted_data) == 4:  # assume 4 digits for years
        year = int(extracted_data)
...
def improved_parse_passport(passport_data):
    byr_index = 0
    iyr_index = 1
    eyr_index = 2
    hgt_index = 3
...
def day_four_part_two(data):
    ...
    for line in data:
        if line != "":
            ...
            passport_list.append(improved_parse_passport(raw_passport_data))
            raw_passport_data = ""
...
# assert day_four_part_two(["pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980", "hcl:#623a2f", ""]) == 1
# assert day_four_part_two(["eyr:2029 ecl:blu cid:129 byr:1989",
#                           "iyr:2014 pid:896056539 hcl:#a97842 hgt:165cm", ""]) == 1
# assert day_four_part_two(["hcl:#888785", "hgt:164cm byr:2001 iyr:2015 cid:88",
#                           "pid:545766238 ecl:hzl", "eyr:2022", ""]) == 1
# assert day_four_part_two(["iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719", ""]) == 1

#assert day_four_part_two(invalid_data) == 0
assert day_four_part_two(valid_data) == 4

day_four_part_two(data)
