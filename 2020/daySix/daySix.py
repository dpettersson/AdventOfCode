...
def day_six_part_one(input_data):
    raw_data = ""
    groups = []
    sums = 0
    ...
    for line in input_data:
        if line != "":
            ...
            groups.append(raw_data)
            raw_data = ""

    for group in groups:
        sums = sums + len("".join(set(group)))

    print("Answer: " + str(sums))

    return sums


assert day_six_part_one(test_data[0]) == 3
assert day_six_part_one(test_data[1]) == 3
assert day_six_part_one(test_data[2]) == 3
assert day_six_part_one(test_data[3]) == 1
assert day_six_part_one(test_data[4]) == 1

assert day_six_part_one(test_data2) == 11

input_file = open("input.txt", "r")
data = input_file.read().splitlines()

day_six_part_one(data)